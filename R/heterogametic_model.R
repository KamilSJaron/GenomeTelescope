nlsLM_2peak_heterogametic_model_fixed_r <- function(x, y, estKmercov, estLength, heterogameticSize, r){
	nlsLM(y ~ (((2*(1-(1-r)^k)) * dnbinom(x, size = kmercov   / bias, mu = kmercov)) +
			   ((1-r)^k)         * dnbinom(x, size = kmercov*2 / bias, mu = kmercov*2)) * (length - hetSize) +
			  dnbinom(x, size = kmercov   / bias, mu = kmercov) * hetSize,
		  start = list(kmercov = estKmercov, bias = 0.5, length = estLength, hetSize = heterogameticSize),
