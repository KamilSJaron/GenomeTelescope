...
#' START_SHIFT -Coverage steps to trim off between rounds
#'
#' @format intiger
#'
"START_SHIFT"

#' TYPICAL_ERROR - Typical cutoff for sequencing error
#'
#' @format intiger
#'
"TYPICAL_ERROR"

#' MAX_ITERATIONS - Max rounds on NLS
#'
#' @format intiger
#'
"MAX_ITERATIONS"

#' SCORE_CLOSE - Overrule if two scores are within this percent (0.05 = 5 per cent) but larger difference in het
#'
#' @format float
#'
"SCORE_CLOSE"

#' SCORE_HET_FOLD_DIFFERENCE - Overrule heterozygosity if there is a large difference in het rate
#'
#' @format intiger
#'
"SCORE_HET_FOLD_DIFFERENCE"

#' VERBOSE - Print out VERBOSEging messages (0/1)
#'
#' @format intiger
#'
"VERBOSE"
...
#' genomescope - GenomeScope wrapper
#'
#' @param kmer_prof - data.frame with kmer coverages and kmer frequencies
#'
#' @param foldername - name of the output
#'
#' @param k - kmer size (defaut 21)
#'
#' @param readlength - read length (defaut 150)
#'
#' @export
genomescope <- function(kmer_prof, foldername, k = 21, readlength = 150){

  maxCovGenomeLen = -1
  if (k > readlength) { stop("K cannot be greater than readlength") }

  cat(paste("GenomeScope analyzing a histogram k=", k, " readlen=", readlength, " outdir=", foldername, "\n", sep=""))
